...
interface RoadmapShareButtonProps {
  roadmapTitle: string;
  url?: string;
  roadmapCreator: string;
}

const RoadmapShareButton = ({
  roadmapTitle,
  ...
  const shareTitle = `Check out this roadmap: ${roadmapTitle}`;
  const shareText = `Discover this amazing roadmap "${roadmapTitle}" by ${roadmapCreator} on HatComics!`;
    ...
    <div className="relative inline-block">
      ...
      {showDropdown && (
        <>
          ...
          {/* Dropdown */}
          <div className="absolute bottom-full left-10 mt-2 w-48 bg-zinc-800/90 backdrop-blur-sm border border-zinc-700/50 rounded-xl shadow-xl z-20">
            <div className="py-2">
              {/* Native Share (mobile) */}
              <button
                onClick={handleNativeShare}
                className="flex items-center gap-3 w-full px-4 py-3 text-left text-zinc-300 hover:bg-orange-500/10 hover:text-orange-300 transition-all duration-200 cursor-pointer"
              >
                <Share2 className="w-4 h-4 flex-shrink-0" />
                <span className="text-sm">Share Roadmap</span>
              </button>

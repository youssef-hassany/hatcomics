...
const FollowersListModal = () => {
  const { isFollowersModalOpen, closeFollowersModal, username } =
    useFollowModalStore();
  const { data: followers, isLoading, error } = useGetFollowers(username);

  return (
    <Modal isOpen={isFollowersModalOpen} onClose={closeFollowersModal}>
